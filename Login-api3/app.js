...
app.post('/bidlist/order',jsonParser, function (req, res, next) {
  connection.execute(
  'INSERT INTO bid (  product_id,  Type, price, amount,  sweets, category) VALUES ( ?,   ?, ?,  ?, ? ,?)',
  [req.body.product_id, 
      req.body.Type, 
      req.body.price, 
      req.body.amount, 
...
app.get('/bidlist/orderlist', function (req, res, next) {
  connection.query(
      'SELECT  product_id, Type, price, amount, sweets, category FROM bid ',
      function(err, results, fields) {
          if (err) {
              res.json({status: 'error', message: err});
              return;
...
//######################## view data order ##########################
// ดึงข้อมูลจาก bidshop
app.get('/bidlist/shops', jsonParser, function (req, res, next) {
  connection.query(
    'SELECT * FROM bidshop', // หรือเลือกเฉพาะคอลัมน์ที่ต้องการ
    function(err, results, fields) {
      if (err) {
        res.json({ status: 'error', message: err });
        return;
...
app.delete('/bidlist/cart/delete',jsonParser, function (req, res, next) {
    connection.execute(
        'DELETE * FROM bidshop WHERE bidshop_id = ?',
        [req.body.bid_id],
    function(err, results,  fields) {
...
app.post('/menu', jsonParser, function (req, res, next) {
  // ตรวจสอบว่า product_id ซ้ำกันหรือไม่
  connection.execute(
    ...
    function (err, results, fields) {
      ...
      if (results.length > 0) {
        // ถ้า product_id ซ้ำกัน ให้ส่งข้อความ error
        res.json({ status: 'error', message: 'Product ID already exists' });
        return;
      }

      // ถ้า product_id ไม่ซ้ำกัน ให้เพิ่มข้อมูลใหม่
      connection.execute(
        'INSERT INTO manu (name_manu, product_id, price, npng, category) VALUES (?, ?, ?, ?, ?)',
        [req.body.name_manu, req.body.product_id, req.body.price, req.body.npng, req.body.category], // บันทึกค่า category
        function (err, results, fields) {
          if (err) {
            res.json({ status: 'error', message: err });
            return;
...
app.post('/manu:pickdrink', jsonParser, function (req, res, next) {
  connection.execute(
    'SELECT * FROM manu WHERE product_id=?',
    [req.body.product_id],
    function (err, results, fields) {
...
app.get('/manu/pickdrink', jsonParser, function (req, res, next) {
  connection.query(
    'SELECT * FROM manu WHERE category="Drink"',
    function (err, results, fields) {
      if (err) {
        res.json({ status: 'error', message: err });
        return;
...
app.post('/manu:pickcake', jsonParser, function (req, res, next) {
  connection.execute(
    'SELECT * FROM manu WHERE product_id=?',
    [req.body.product_id],
    function (err, results, fields) {
...
app.get('/manu/pickcake', jsonParser, function (req, res, next) {
  connection.query(
    'SELECT * FROM manu WHERE category="Cake"',
    function (err, results, fields) {
      if (err) {
        res.json({ status: 'error', message: err });
        return;
